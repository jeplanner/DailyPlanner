...
-- ============================================================
--  Quick Bucket -> calendar slot
--
--  Bulk-selecting bucket items and dropping them into one calendar
--  slot needs somewhere to record WHERE they went. Without it the
--  only options are to delete the items (losing them) or to mark them
--  done (which is a lie — they are scheduled, not finished).
--
--  `scheduled_event_id` points at the daily_events row that now
--  carries them; `scheduled_for` is that slot's date, denormalised so
--  the bucket list can show "on the calendar Fri 3pm" without joining.
