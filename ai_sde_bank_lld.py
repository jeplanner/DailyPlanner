...
'''),
          ...
          examples=[
              "The if/elif that is secretly a Strategy. `def cost(order): if order.type == 'standard': return 5 + 0.5*w; elif order.type == 'express': return 15 + 1.2*w + 0.05*d; elif ...`. Every new shipping option edits this function, so every option's team touches one file and can break the others, and the function grows without bound. Extract each branch into a class implementing cost(), and adding FreeShipping becomes a new file plus one line of wiring. The recognition rule: an if/elif chain that PICKS A BEHAVIOUR (rather than validating input) is a Strategy waiting to be extracted.",
              "Runtime swapping, which is what inheritance cannot give you. `order.set_shipping(ExpressShipping())` changes the pricing rule on an object that already exists, mid-transaction, because the user upgraded at checkout. An OrderWithStandardShipping subclass is fixed at construction - you would have to build a new object and copy its state across. Any requirement containing 'configurable', 'the admin can change', or 'depending on the customer tier' is telling you to compose a strategy rather than subclass.",
              "Strategy in the standard library, so you can point at something familiar. `sorted(people, key=lambda p: p.age)` injects the ordering rule - sorted() supports orderings its authors never imagined, without a single if/elif inside it. Same for `defaultdict(list)` (the default-value strategy), `retry(backoff=exponential)` and any function taking a comparator or a callback. In Python a strategy is often just a FUNCTION rather than a class, and saying that shows you know the pattern rather than the ceremony.",
              "How the caller chooses, which is the follow-up. You have not deleted the if/elif, you have QUARANTINED it - and the right place for it is a small factory or a registry dict: `STRATEGIES = {'standard': StandardShipping, 'express': ExpressShipping}` then `STRATEGIES[name]()`. One lookup, one place to change, and a plug-in can register itself without editing your file. Being explicit that the branching moved rather than vanished is a more honest answer than pretending Strategy eliminates it.",
              "The design smell to avoid: leaky interfaces. If ExpressShipping needs a delivery window and FreeShipping needs a coupon code, the temptation is to widen cost() to cost(weight, distance, window, coupon) - and now every strategy takes parameters it ignores, which is Interface Segregation being violated one argument at a time. The fix is to pass a single context object (the Order) and let each strategy read what it needs, or to hold strategy-specific configuration in the strategy's own constructor.",
              "When NOT to use it. One implementation and no concrete plan for a second means a strategy class is pure indirection: a reader now jumps to a second file to find one formula. The honest interview line is 'I'd keep this inline today, and extract a Strategy the moment we have a second pricing rule - which the roadmap says is next quarter.' Knowing when a pattern is premature scores higher than applying it everywhere, and interviewers do probe for over-engineering.",
          ],
...
'''),
          ...
          examples=[
              "What the factory actually buys you, counted. Fifty files call `StripeClient(api_key, region, retries=3, timeout=5)` directly. Now the constructor gains a required `webhook_secret` - fifty edits, fifty chances to get it wrong, and any file that is missed fails at runtime rather than at import. Route construction through one factory and the change is one edit. The general principle: knowing HOW to build something is a dependency just like knowing how to use it, and it should live in one place.",
              "Why a registry dict beats an if/elif chain. `create(channel)` with branching means every new channel edits the same method, so two teams adding channels in the same sprint conflict. With `_registry = {'email': EmailNotification, ...}` adding a channel is one dict entry - and with the `register()` classmethod, a channel module can add ITSELF at import time, so the factory file is never touched at all. That is how plug-in systems and Django's app registry work.",
              "Factories you already use. `json.loads('{...}')` returns a dict, `json.loads('[1,2]')` a list, `json.loads('3')` an int - you ask for a Python object and it decides the concrete type. `sqlalchemy.create_engine('postgresql://...')` picks a Postgres dialect and connection class from the URL scheme; change the scheme to `mysql://` and everything downstream is unchanged. Pointing at a familiar example lands better than inventing a ShapeFactory.",
              "Failing loudly, which is the detail juniors miss. `return cls._registry.get(channel)` returns None on a typo, and the caller then hits `AttributeError: 'NoneType' has no attribute 'send'` three frames away with no clue what was misspelled. Raising `ValueError(f\"unknown channel {channel!r}; expected one of {sorted(registry)}\")` turns a config typo into a message that names the mistake and lists the valid options. Construction is exactly where errors should be loud, because it is the last point with enough context to explain them.",
              "Factory Method versus a simple factory, which is the terminology question. A SIMPLE factory is one function or classmethod holding the branching - what the code above does, and what you will actually write. FACTORY METHOD is the Gang-of-Four pattern: the base class defines an abstract `create_x()` and each SUBCLASS decides what to build, so a base algorithm can operate on products it has never heard of (a Dialog base class whose `create_button()` is implemented by WindowsDialog and WebDialog). Say which one you mean; interviewers sometimes probe the distinction.",
              "When it is over-engineering. Two implementations and no third on the horizon means the factory is a layer of indirection buying nothing - the reader now jumps to another file to learn that 'email' means EmailNotification. The honest line: 'I'd construct these directly today and add a factory when a third channel appears or when the construction arguments grow.' Interviewers probe for premature abstraction just as often as for missing abstraction.",
          ],
...
'''),
          ...
          examples=[
              "A full transaction, traced through the states. Machine in Idle. select('A1') -> Idle.select checks stock, sets m.selected, returns HasMoney. insert(100) -> inserted 100 < price 150, stays HasMoney. insert(100) -> inserted 200 >= 150, change 50 needed, plan_change(50) succeeds, returns Dispensing([50]). The machine calls finish(): stock decrements, the 50c coin leaves the bank, state returns to Idle. Every transition is a return value from the current state, so the legal moves are visible in one place rather than scattered across if-blocks.",
              "The change-making refusal, which is the detail that separates answers. Item costs 150, user inserts 200, machine holds no 50c coins. plan_change(50) walks 200, 100, 50, 20, 10 and cannot make exactly 50 from what it holds, so it returns None - and the machine REFUNDS the 200 rather than dispensing and short-changing the customer. That is what real machines do, and checking feasibility BEFORE committing the sale is the same order-of-operations discipline as the ATM's plan-before-debit.",
              "Why greedy change works here and the caveat to state anyway. For real currency sets (200/100/50/20/10) greedy from the largest coin is optimal - the denominations are canonical. For a made-up set like {1, 3, 4}, greedy makes 6 as 4+1+1 (three coins) when 3+3 (two coins) is better. Say this out loud even though it does not bite: it shows you know WHY the simple algorithm is safe here rather than assuming it always is, and the interviewer may follow up with exactly that set.",
              "What the naive design looks like, for contrast. `self.status = 'idle'` plus every method opening with `if self.status == 'idle': ... elif self.status == 'has_money': ...`. It works for three states. Then cancel-during-dispensing appears, and sold-out, and card payment, and now four methods each carry a five-branch chain that must be kept mutually consistent by hand. The legal transitions exist only in the author's head, so an illegal one is a bug you can only find by testing. With state classes, `Dispensing.select` simply raises - illegal transitions are impossible rather than merely tested for.",
              "The state diagram to draw before coding, which takes twenty seconds. Idle --select--> HasMoney --insert(enough)--> Dispensing --finish--> Idle, with HasMoney --cancel--> Idle (refund) and Idle --select(empty)--> stays Idle (sold out). Drawing it makes the code fall out, and it makes the interviewer's follow-ups easy to answer because you can point at the edge that would change. Candidates who code first and diagram never are the ones who miss cancel-during-dispensing.",
              "Adding card payment - the follow-up that tests the design. If the design is right, the state machine does not change at all: HasMoney only cares whether the balance covers the price, so you introduce a PaymentMethod interface with CoinPayment and CardPayment, and HasMoney asks the payment method rather than counting coins. If instead card handling requires editing three state classes, the states were tracking payment mechanics rather than transaction phase - which is the diagnosis to offer if you find yourself in that position.",
          ],
