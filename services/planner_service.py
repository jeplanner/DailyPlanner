...
def get_morning_dashboard(plan_date, user_id):
    """Morning dashboard payload for /summary?view=daily.

    Thin wrapper — the composition logic now lives in
    services.agenda_service.build_dashboard. Keeping the symbol here so
    existing imports from services.planner_service keep working.
    """
    from services.agenda_service import build_dashboard
    return build_dashboard(user_id, plan_date)
